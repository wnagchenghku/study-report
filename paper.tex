...
\newcommand{\mytitle}[0]{\textbf {Problems on providing efficient fault 
tolerance to server programs}}
...
three problems as elaborated below.

\subsection*{Issue 1: High Consensus Latency}

The consensus latency of traditional \paxos 
...
ping RTT in LAN typically takes hundreds of \us, and that the request 
processing 
...
\subsection*{Issue 2: Poor scalability}
...
The consensus latency of extant consensus protocols 
...
\subsection*{Issue 3: Hard to use}
Most existing \smr systems are not designed to 
...
programs into the file IO interface defined by \zookeeper. As an another 
example, in order to provide fault tolerance to a whole virtual machine, most 
of the existing \smr systems need to modify the hypervisor code which can be 
complex and error-prone.

Although there are some hardware-accelerated consensus 
protocols~\cite{consensusbox:nsdi16,netpaxos:sosr15,specpaxos:nsdi15,
nopaxos:osdi16} which are effective on reducing consensus latency, they are 
either unsuitable for general server programs or are not designed to be 
scalable on concurrent client connections. For instance, 
\dare~\cite{dare:hpdc15}, a novel consensus protocol, achieves the lowest 
consensus latency on a small number of client connections, but its 
evaluation shows that its consensus latency increases quickly when  
more connections are added. Other recent 
works~\cite{nopaxos:osdi16,specpaxos:nsdi15,netpaxos:sosr15} 
leverage in-network serialization in a datacenter to safely skip 
consensus if packets arrive at replicas in the same order. 
These works require reconstructing a server program to use their new libraries 
for checking the order of packets, so they are not designed to run legacy 
server programs.
